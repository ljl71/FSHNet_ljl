...
from .augmentor.data_augmentor_mf import DataAugmentor_mf
from .processor.data_processor_mf import DataProcessor_mf
from .processor.point_feature_encoder import PointFeatureEncoder


class DatasetTemplate_mf(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        self.data_augmentor = DataAugmentor_mf(
            self.root_path, self.dataset_cfg.DATA_AUGMENTOR, self.class_names, logger=self.logger
        ) if self.training else None
        self.data_processor = DataProcessor_mf(
    ...
    def prepare_data(self, data_dict):
        ...
        if 'num_points_all' in data_dict:
            previous_frames = []
            counts = 0
            points = data_dict['points']
            for i in range(len(data_dict['num_points_all'])):
                previous_frames.append(points[counts:counts+data_dict['num_points_all'][i]])
                counts += data_dict['num_points_all'][i]

            data_dict['points'] = previous_frames
            # data_dict.pop('num_points_all')
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, val in data_dict.items():
            try:
                ...
                elif key in ['points', 'multi_points', 'voxel_coords']:
                    coors = []
                    if isinstance(val[0], list):
                        batch_size_ratio = len(val[0])
                        val =  [i for item in val for i in item]
