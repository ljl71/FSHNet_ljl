...
from pcdet.models.model_utils.vis_utils import draw_scenes

class DataAugmentor_mf(object):
    ...
    def random_world_flip(self, data_dict=None, config=None):
        ...
        if isinstance(data_dict['points'], list):
            ...
            for i in range(len(points)):
                ...
                draw_scenes(points[i])
            points = np.concatenate(data_dict['points'], axis=0)
            colors = np.concatenate([np.ones((num_points_all[0], 3), dtype=np.float), np.ones((num_points_all[1], 3), dtype=np.float) * 0.5], axis=0)
            draw_scenes(points, point_colors=colors)
