...
from pcdet.models.model_utils.pruning_block import SpatialPrunedConv2d
...
class SparseHEDNet_slotformer(nn.Module):
    ...
    def __init__(self, model_cfg, input_channels, grid_size, class_names, voxel_size, point_cloud_range, **kwargs):
        ...
        self.upformer_stage0 = spconv.SparseSequential(
            UpFormerLayer(afd_dim, 4, num_layers=2, win_size=12, shift=True, cwi=True), 
            nn.BatchNorm1d(afd_dim, eps=1e-3, momentum=0.01),
        )
        ...
        self.upformer_stage1 = spconv.SparseSequential(
            UpFormerLayer(afd_dim, 4, num_layers=2, win_size=12, shift=True, cwi=True), 
            nn.BatchNorm1d(afd_dim, eps=1e-3, momentum=0.01),
        )

        self.upformer_stage2 = spconv.SparseSequential(
            UpFormerLayer(afd_dim, 4, num_layers=2, win_size=12, shift=True, cwi=True), 
            nn.BatchNorm1d(afd_dim, eps=1e-3, momentum=0.01),
        )
    ...
    def forward(self, batch_dict):
        ...
        x = self.upformer_stage0(x)

        x = self.upformer_stage1(x)
