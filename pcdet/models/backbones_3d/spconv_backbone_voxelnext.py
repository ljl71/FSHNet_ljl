...
# from pcdet.models.model_utils.hednet_utils import post_act_block_sparse_3d, post_act_block_sparse_2d, post_act_block_dense_2d
# from pcdet.models.model_utils.up_former_utils import UpFormerLayer
...
class VoxelResBackBone8xVoxelNeXt(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        # self.decode_layer3 = post_act_block_sparse_3d(channels[4], channels[4], kernel_size=3, stride=2, \
        #                       padding=1, conv_type='inverseconv', indice_key='spconv6')
        
        # self.decode_layer2 = post_act_block_sparse_3d(channels[4], channels[3], kernel_size=3, stride=2, \
        #                       padding=1, conv_type='inverseconv', indice_key='spconv5')
        
        # self.norm_layer3 = norm_fn(channels[4])

        # self.norm_layer2 = norm_fn(channels[3])

        # self.up_block = spconv.SparseSequential(
        #     post_act_block_sparse_2d(channels[3], channels[3], kernel_size=3, stride=1, \
        #                       padding=1, conv_type='spconv', indice_key='up_block'),
        # )

        # self.upformer = spconv.SparseSequential(
        #     UpFormerLayer(channels[3], 4, num_layers=2, win_size=12), 
    ...
    def upsampling(self, x):
        ...
        #     spatial_shape=[s * up_stride for s in x.spatial_shape],
        #     batch_size=x.batch_size
        # )
        
        x.indices[:, 1:] *= 2
        x.spatial_shape = [s * 2 for s in x.spatial_shape]
    ...
    def forward(self, batch_dict):
        ...
        # x_conv5_up = self.decode_layer3(x_conv6)
        # x_conv5_up = x_conv5.replace_feature(self.norm_layer3(x_conv5_up.features + x_conv5.features))

        # x_conv4_up = self.decode_layer2(x_conv5_up)
        # x_conv4_up = x_conv4.replace_feature(self.norm_layer2(x_conv4_up.features + x_conv4.features))

        out = self.bev_out(x_conv4)

        # out = self.upformer(out)

        # out = self.upsampling(out)
        out = self.conv_out(out)
        out = self.shared_conv(out)

        batch_dict.update({'spatial_features_2d': out})
